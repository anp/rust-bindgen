...
impl Bindings {
    ...
    pub(crate) fn generate(
        ...
        // depfiles need to include the explicitly listed headers too, collect them into a temp
        // vec so that we can mutate the context where they're stored
        let mut explicitly_included = vec![];
        if let Some(filename) = &context.options().input_header {
            explicitly_included.push(filename.to_owned());
        }
        ...
        for filename in explicitly_included {
